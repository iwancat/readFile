...
def col_spark():
    ...
    result = hc.sql("create table stat_aaa_2022030901 as select count(1) from userlist_4")
    print("===========")
    print(result)
    print("===========")
    result.show()
...
def col_spark2():
    conf2 = SparkConf().setAppName("AA" + "bb" + "2")
    sc2 = SparkContext(conf=conf2)
    hc2 = HiveContext(sc2)
    hc2.setConf('hive.exec.dynamic.partition.mode', 'nonstrict')
    #    sqlong = "create table stat_ccc_sp_20220310 as select * from 表 where arrive_time>=20210927170000 and arrive_time<20210927180000"
    sqlong = "select mmonth,count(1) ddct from stat_ccc_20220310 group by  mmonth"
    result2 = hc2.sql(sqlong)
    sslist =  result2.toPandas()

    #filepa = "/home/hadoop/anhy/spark/slisresult.csv"  #屏蔽写本地
    # ofnt = open(filepa, "w+")       #屏蔽写本地
    hdfile = []
    #ofnt = open(filepa, "w+")        #屏蔽写本地

    for slist in sslist.itertuples():
        linetxt= str(getattr(slist, 'ddd'))+"=====" + str(getattr(slist, 'vvv')) + "G000000G" + str(getattr(slist, 'hhh')) + str(getattr(slist, 'ddct'))+"\n"
        #   ofnt.writelines(linetxt)  #屏蔽写本地
        hdfile.append(linetxt)
    #ofnt.close()                     #屏蔽写本地

    print("next is hdfs write")
    hd_path = "/qwmonitor/100-001-002.csv"
