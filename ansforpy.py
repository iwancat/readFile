...
if __name__ == '__main__':
    ...
    # 进程号，cpu，内存
    ansible2.run(hosts= "myserver", module="shell", args="ps aux |grep -v grep|grep Dproc_journalnode | awk '{print $2,$3,$6}'" )
    ...
    succ = psData['success']
    print("节点IP", "进程ID", "CPU使用率", "使用内存", "状态")
    for suc in succ :
        sss_list = succ[suc]
        if sss_list['stdout'] == '':
            stat_sg = "abort"
            print(suc, "null", "null", "null", stat_sg)
        else:
            stat_sg = "running"
            print(suc, sss_list['stdout'], stat_sg)
